...
#define DT_DRV_COMPAT zmk_behavior_input_two_axis_oneshot
...
struct behavior_input_two_axis_config {
    ...
    // ★追加
    bool oneshot_on_press;
    int16_t oneshot_scale;
    bool disable_continuous;
};
...
static void tick_work_cb(struct k_work *work) {
    ...
    if (have_x) {
        input_report_rel(dev, cfg->x_code, (int16_t)CLAMP(move.x, INT16_MIN, INT16_MAX),
                         !have_y, K_NO_WAIT);
    }
    if (have_y) {
        input_report_rel(dev, cfg->y_code, (int16_t)CLAMP(move.y, INT16_MIN, INT16_MAX),
                         true, K_NO_WAIT);
    }
...
// ★追加：press時にワンショット送信
static void oneshot_emit(const struct device *behavior_dev,
                         const struct behavior_input_two_axis_config *cfg,
                         int16_t x, int16_t y) {
    if (!cfg->oneshot_on_press) {
        return;
    }

    int32_t sx = (int32_t)x * (int32_t)cfg->oneshot_scale;
    int32_t sy = (int32_t)y * (int32_t)cfg->oneshot_scale;

    int16_t out_x = (int16_t)CLAMP(sx, INT16_MIN, INT16_MAX);
    int16_t out_y = (int16_t)CLAMP(sy, INT16_MIN, INT16_MAX);
    ...
    // syncは最後だけtrue
    if (have_x) {
        input_report_rel(behavior_dev, cfg->x_code, out_x, !have_y, K_NO_WAIT);
    }
    if (have_y) {
        input_report_rel(behavior_dev, cfg->y_code, out_y, true, K_NO_WAIT);
    }
}
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    // ★tapでも確実に1回送る
    oneshot_emit(behavior_dev, cfg, x, y);

    // ★連続スクロール無効ならここで終わり
    if (cfg->disable_continuous) {
        return 0;
    }
...
#define ITA_INST(n)                                                                                 \
    ...
    static struct behavior_input_two_axis_config behavior_input_two_axis_config_##n = {             \
        ...
        .oneshot_on_press = DT_INST_PROP_OR(n, oneshot_on_press, 1),                                \
        .oneshot_scale = DT_INST_PROP_OR(n, oneshot_scale, 120),                                    \
        .disable_continuous = DT_INST_PROP_OR(n, disable_continuous, 1),                            \
    };                                                                                              \
